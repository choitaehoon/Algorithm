...
public class 프린터큐 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i=0; i<testCase; ++i) {
            ...
            for (int j=0; j<n; ++j)
                queue.offer(new Pair(j,Integer.parseInt(importanceToken.nextToken())));

            System.out.println(findOutHowManyTimes(queue));
        }
    }

    public static int findOutHowManyTimes(Queue<Pair> queue) {
        int sum = 1;
    ...
    public static boolean identifyHigherImportance(Queue<Pair> queue) {
